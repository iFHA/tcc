...
Como o Java EE é apenas uma especificação, fez-se o uso do framework Jersey, que implementa a especificação JAX-RS, referente à criação e utilização de serviços web na plataforma Java, a figura a seguir exibe as dependências Java adicionadas ao arquivo pom.xml, utilizadas neste trabalho.
Para o desenvolvimento do middleware, foram utilizadas as tecnologias: Java - como linguagem padrão da implementação; Jersey - como framework para criação do webservice; Maven - como ferramenta de automatização e gerenciamento de projetos; JSON - como forma padrão de comunicação entre servidor e cliente; codificação base64 - para facilitar a transferência de arquivos binários(imagens, no contexto deste trabalho).
...
Assim sendo, na linha 22, têm-se a definição de qual serviço o usuário escolheu, atribuindo-o à variável serviceUrl, que mais tarde será utilizada para redirecionar a requisição para o serviço correto. A Figura \ref{fig:Figura6} exibe as URLs associadas a cada serviço do webservice. Como o middleware está localizado na máquina local, seu acesso se tem por meio da URL \url{http://localhost:8080/demorest/restapi/ws}; 
