...
O principal objetivo do WS desenvolvido é possibilitar aos desenvolvedores uma forma fácil e intuitiva de utilizar as funcionalidades da API apresentada na seção 2.4 por meio de requisições HTTP. Para isso, o programador não deverá encontrar dificuldades em adaptar a sua forma de desenvolvimento atual para uma arquitetura de comunicação distribuída, uma vez que praticamente todas as linguagens de programação possuem alguma API específica para auxiliar na realização de chamadas a WS's RESTful. O estudo de caso mostrou resultados promissores na adoção de serviços web RESTful para encapsular e disponibilizar as funções da API. Dentre as principais vantagens, pode-se mencionar a redução da dificuldade de acesso a funções de processamento de imagens digitais, bem como a interoperabilidade entre linguagens de programação que essa tecnologia proporciona.
